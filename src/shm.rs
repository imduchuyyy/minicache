//! A cache that lives in a memory-mapped file, so several processes on the same
//! host share one cache with no server and no network hop.
//!
//! Everything inside the mapping is `#[repr(C)]` with a fixed layout. Nothing in
//! there may be a pointer: each process maps the file at a different address, so a
//! pointer written by one process is meaningless to another. That rules out reusing
//! [`crate::LruCache`], whose `HashMap`/`Vec`/`Bytes` are all process-local heap.
//!
//! # Layout
//!
//! ```text
...
//! behaviour, so there is no LRU list — maintaining one across processes needs shared
//! locking, which is exactly what this design avoids.
//!
//! # Concurrency
//!
//! Each slot is a seqlock. A writer bumps `seq` to odd, writes, then stores it even
//! again; a reader samples `seq`, copies the value out, and re-reads `seq`, retrying
//! if it moved. The copy is what makes the re-read meaningful — see [`ShmCache::read`].
//!
//! There are no process-shared mutexes, so a process dying mid-write cannot poison or
//! deadlock the cache. It leaves one slot with an odd `seq`; both sides spin a bounded
//! number of times, so that slot degrades to a miss rather than wedging a caller
...
/// A handle onto a shared-memory cache. Cloning the process (fork/exec) and opening
/// the same path again gives another handle onto the same cache.
pub struct ShmCache {
    mmap: MmapMut,
    num_slots: usize,
}
...
impl ShmCache {
    ...
    fn init_header(&self, num_slots: usize) -> Result<(), Error> {
        ...
        {
            Ok(_) => {
                // We created it. The file is already zeroed by the OS, so the slots
            ...
            Err(MAGIC) => {
                // Someone else got there first; wait for them to publish `ready`.
    ...
    pub fn write(&self, key: &[u8], value: &[u8]) -> Result<(), Error> {
        ...
        // Exclusive access. Readers that sample an odd seq will retry, and readers
        // that sampled before we locked will notice seq changed and discard.
    ...
    pub fn read(&self, key: &[u8]) -> Option<Bytes> {
        ...
        loop {
            ...
            fence(Ordering::Acquire);
            if slot.seq.load(Ordering::Acquire) != before {
                // Overwritten while we were copying; the bytes we have are a mix of
                // two values, so discard them.
                if !backoff.wait(READ_LOCK_TIMEOUT) {
                    return None;
                }
                continue;
